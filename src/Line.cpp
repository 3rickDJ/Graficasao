...
void Line::draw() { Line::Draw(pe1, pe2); }
...
void Line::Draw(Point p1, Point p2) {
    ...
    dy = p2.y - p1.y;
    // Si la diferencial de x2 - x1 es menor que 0, la diferencial tomara su
    // valor en positivo
    if (dx < 0)
        dx = -dx;
    // Si la diferencial de y2 - y1 es menor que 0, la diferencial tomara su
    // valor en positivo
    if (dy < 0)
        dy = -dy;
    ...
    if (dx > dy) // Caso 1
    {
        ...
        inc1 = 2 * (dy - dx);
        inc2 = 2 * dy;
        // Lo valores solo se necesitan calcular una vez, ya que el incremento
        // sera el mismo.
        for (i = 0; i < dx; i++) {
            // Se opta por dibujar el pixel en el punto medio superior si la
            // recta queda exactamente en el punto medio
            if (e >= 0) {
                p.y += incy;
                e += inc1;
            } else
                e += inc2;
            /*La siguiente coordenada X a ser dibujada sera definida ya sea 1 o
            -1 dependiendo la posicion de punto 2 en el eje X */
            p.x += incx;
            p.draw();
        }
           ...
           // entre los puntos Y's
    {
        ...
        for (i = 0; i < dy; i++) {
            if (e >= 0) {
                ...
                e += inc2;
            /*La siguiente coordenada Y a ser dibujada sera definida ya sea 1 o
             -1 dependiendo la posicion de punto 2 en el eje Y */
            p.y += incy;
            p.draw();
        }
    }
